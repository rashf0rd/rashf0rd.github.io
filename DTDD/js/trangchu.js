window.onload = function () {
	...
	addBanner("image/banner0.gif", "image/banner0.gif");
	var numBanner = 9; // Số lượng hình banner
	for (var i = 1; i <= numBanner; i++) {
		var linkimg = "image/banner" + i + ".png";
	...
	for (var t of tags) addTags(t, "applications.html?search=" + t);
	...
	for (var c of company) addCompany("image/company/" + c, c.slice(0, c.length - 4));
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
	...
	s += `	</div>
			<a class="xemTatCa" href="applications.html?` + filter.join('&') + `" style="` + borderA + `">
...
function createLinkFilter(type, nameFilter, valueAdd) {
	...
	var link = 'applications.html'; //window.location.href.split('?')[0].replace('#', '');
